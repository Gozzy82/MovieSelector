...
let movieTitleGrabber = function(array) {
    return array.map(function (movies) {
        return movies.title;
    })
}

let allMovieTitles = movieTitleGrabber(allMovies)

Array.from(allMovieTitles);

let xmenFilter = function(array) {
    array.filter(function (movietitle) {
        if (movietitle.includes('X-Men')) {
            console.log('Including', movietitle, '...')
            return movietitle;
        } else {
            return;
        }
...
console.log(xmenFilter(allMovieTitles))
