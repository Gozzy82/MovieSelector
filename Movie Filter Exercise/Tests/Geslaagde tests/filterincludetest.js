...
let allMovieTitles = allMovies.map (function (movie) {
    return movie.title;
})

let allNewMovies = allMovieTitles.filter(
    function(title) {
        parseInt(title.year);
        return title.year > 2013;
    }
)

let allXmenMovies = allMovieTitles.filter(
    function(title) {
        return title.includes('X-Men')
    }
)

let allAvengerMovies = allMovieTitles.filter(
    function(title) {
        return title.includes('Avenger')
    }
)

let allBatmanMovies = allMovieTitles.filter(
    function(title) {
        return title.includes('Batman')
    }
)

let allPrincessMovies = allMovieTitles.filter(
    function(title) {
        return title.includes('Princess')
    }
)

/*
console.log('All movie titles:', allMovieTitles);
console.log('X-men titles', allXmenMovies)
console.log('Avenger titles', allAvengerMovies)
console.log('Batman titles', allBatmanMovies)
console.log('Princess titles', allPrincessMovies)
console.log(allNewMovies)
*/

let altXmenFilter = function(array) {
    return array.filter(function(title) {
        return title.includes('X-Men');
    })
}

console.log(altXmenFilter(allMovieTitles))
